...
#define	ZS_PROP_FILE  "./briano/zs_sample.prop"	//Configuration file
...
main( )
{
	...
	props.size_kb = CNTR_SIZE*1024 *1024;
	props.flash_only = 0;
	// props.flags=2;
	props.flags=0;
	...
	for (int i=0; i< 5000; i++) {
		status = ZSWriteObject(thd_state, cguid, "test", 9, "testdata", 4, 0);
	}
...
worker(void *arg)
{
	...
	memset(keyw, 0, keylen);
	memset(dataw, 0, DATA_SIZE+5);
	int i;
