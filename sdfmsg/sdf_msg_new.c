...
sdf_msg_init(int argc, char *argv[])
{
    ...
    if (!(Conf.set & MP_RANK))
        Conf.myrank = -1;
    zs_logi(70029, "sdfmsg version %d", VER_SDFMSG);
...
set_rank(void)
{
    ...
    for (i = 0;; i++) {
        ...
        if (node_line_me(line)) {
            zs_logi(70030, "found rank of %d in property file", i);
            Conf.myrank = i;
            Conf.set |= MP_RANK;
            break;
        }
...
{
    zs_loge(70031, "bad messaging parameter: %s = %s", name, data);
    return 0;
}
    ...
    if (type == 'd') {
        ...
        if (size == sizeof(int8_t))
            ...
            *((int64_t *) ptr) = str2int(val, parm->name);
            ...
            zs_loge(70032, "parm_set: bad size: type=%c size=%d", type, size);
            return 0;
        }
    } else if (type == 'p') {
        ...
        *((ntime_t *) ptr) = str2time(val, parm->name);
        ...
        zs_loge(70033, "parm_set: bad parameter %s = %s", parm->name, val);
        return 0;
    }
...
parm_show(void)
{
    ...
    for (; n--; parm++) {
        ...
        if (type == 'i' || type == 'u') {
            ...
            zs_logi(70034, "%s = %ld", parm->name, val);
        } else if (type == 't') {
            double secs = (double) *((ntime_t *) ptr) / NANO;
            zs_logi(70035, "%s = %g", parm->name, secs);
        } else if (type == 'p') {
            zs_logi(70036, "%s = %s", parm->name, *((char **)ptr));
        }
    }
}
...
wdog_rnodes(void)
{
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        if (n && buf[n-1] == '\n')
            buf[n-1] = '0';
        zs_logi(70061, "watchdog: R: %s", buf);
        n = strtol(buf, &ptr, 10);
        if (ptr == buf || *ptr != ' ')
...
wdog_send(char *fmt, ...)
{
    ...
    if (fp)
        zs_logi(70062, "watchdog: W: %s", buf);
    else
        zs_logi(70063, "watchdog: connect failed: %s", buf);
    return fp;
}
             ...
             sdf_fth_mbx_t *sfm, sdf_resp_mbx_t *srm)
{
    ...
    if (drank >= V.live.n || !((char *)V.live.p)[drank]) {
        if (msg_rank2nno(drank) < 0) {
            ...
            zs_logi(70037, "sdf_msg_send to dead node: %d", drank);
...
do_recv1(msg_info_t *info)
{
    ...
    if (rank < 0) {
        zs_logi(70038, "received message from unknown node m%d", info->nno);
        goto drop;
    }

    if (info->len < sizeof (sdf_msg_t)) {
        zs_logi(70039, "received truncated message from node n%d", rank);
        goto drop;
    }
    ...
    if (msg->cur_ver != VER_SDFMSG) {
        zs_logi(70040, "received message from node n%d: "
...
do_recv2(msg_info_t *info)
{
    ...
    zs_logi(70041, "stray message: addr=(%d:%d<=%d:%d) id=%ld len=%d",
...
drop_node(int nno)
{
    zs_logi(70042, "bad data from n%d(%d); dropped", msg_nno2rank(nno), nno);
    msg_nodedrop(nno);
}
...
readq_add(readq_t *readq)
{
    ...
    for (q = ReadQ.list; q; q = q->next) {
        if (readq_match(q, readq->srank, readq->sserv, readq->dserv))
            zs_logi(70043, "creating queue=(%d:%d=>%d:%d) "
...
recv_err(sdf_msg_t *msg, char *fmt, ...)
{
    ...
    va_end(alist);
    zs_loge(70044, "%s: addr=(%d:%d=>%d:%d)", (char *)xstr.p,
