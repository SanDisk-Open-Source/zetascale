...
/*
 * File:   fcnl_multinode_lib2.c
 * Author: mac
 *
 * Created on Aug 22, 2008, 1:28 AM
 *
 ...
 *
 * @brief: 1 -> x (singleside, singleprotocol)
 * node 0 create only one type of protocol sender, other nodes create only the same type 
...
void
fthThreadMultiNodeSstomrSender(uint64_t arg) {
    int l = 0;
    vnode_t node;
...
void fthThreadMultiNodeSstomrRecver(uint64_t arg) {
    int ret, ct = 0;
    uint64_t aresp = 0;
    ...
    // I have to wait for the sender finished sending message, otherwise there are so many unexpect message.
    //sleep(2);
    fthKill(1); 
}

void *
MultiNodeSstomrPthreadRoutine(void *arg) {
    ...
    if(myid) {
        fthResume(XSpawn(&fthThreadMultiNodeSstomrRecver, 40960), ptl);
    }
    else {
        for(int index = 1; index < *numprocs; index ++)
            fthResume(XSpawn(&fthThreadMultiNodeSstomrSender, 40960), index);
    }
    info->fth_info = *numprocs + 1;
