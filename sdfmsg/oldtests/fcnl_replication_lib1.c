 ...
 *
 * $Id: fcnl_replication_lib1.c 308 2008-06-23 22:34:58Z normanxu $
 */
...
/*
 * This pthread will be used to simulate the SDF_REPLICATION sending and
...
void * ReplicationPthreadRoutine(void *arg) {
    ...
    while (!q_pair_REPLICATION) {
        ...
        q_pair_REPLICATION = local_create_myqpairs_with_pthread(my_protocol,
                myid, node);
        info->queue_pair_info->queue_add[0] = q_pair_REPLICATION->q_in;
        info->queue_pair_info->queue_add[1] = q_pair_REPLICATION->q_out;
