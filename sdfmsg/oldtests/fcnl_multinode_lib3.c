...
/*
 * File:   fcnl_multinode_lib3.c
 * Author: mac
 *
 * Created on Aug 22, 2008, 4:25 PM
 *
...
void
fthThreadMultiNodeMstosrSender(uint64_t arg) {
    int l = 0;
    vnode_t node;
    ...
    info->queue_pair_info->queue_add[0] = q_pair[myid]->q_in;
    info->queue_pair_info->queue_add[1] = q_pair[myid]->q_out;
...
void fthThreadMultiNodeMstosrRecver(uint64_t arg) {
    int ret, ct = 0;
    uint64_t aresp = 0;
...
void *
MultiNodeMstosrPthreadRoutine(void *arg) {
    ...
    if(myid) {
        ...
        fthResume(XSpawn(&fthThreadMultiNodeMstosrSender, 40960), ptl);
    }
    else {
        // node 0 do the receiver
        for(int index = 1; index < *numprocs; index ++)
            fthResume(XSpawn(&fthThreadMultiNodeMstosrRecver, 40960), index);
    }
