 ...
 *
 * $Id: fcnl_flush_lib1.c 308 2008-06-23 22:34:58Z normanxu $
 */
...
void * FlushPthreadRoutine(void *arg) {
    ...
    plat_log_msg(
            ...
            "\nNode %d: Testing pthread MESSAGING SDF_FLSH protocol communication\n",
            myid);
    ...
    while (!q_pair_FLSH) {
    	...
    	info->queue_pair_info->queue_add[0] = q_pair_FLSH->q_in;
    	info->queue_pair_info->queue_add[1] = q_pair_FLSH->q_out;
    	info->queue_pair_info->queue_pair_type = my_protocol;
        if (!q_pair_FLSH) {
            fprintf(stderr,
                    "\nNode %d: Cannot find qpair %p sn %d dn %d ss %d ds %d\n",
                    myid, q_pair_FLSH, myid, myid == 0 ? 1 : 0, my_protocol,
                    SDF_FLSH);
			return ((void *) 1);
        } else {
            plat_log_msg(
