 ...
 *
 * $Id: membership_test.c 308 2008-05-09 22:34:58Z tomr $
 */
...
void * MembershipPthreadRoutine(void *arg) {
	int myid = *(int *)arg;
	struct sdf_queue_pair *q_pair_MEMBERSHIP;
	...
	while (!q_pair_MEMBERSHIP) {
		usleep(10000);
		printf("before alloc\n");
		q_pair_MEMBERSHIP = local_create_myqpairs_with_pthread(my_protocol, myid, node);
		if (!q_pair_MEMBERSHIP) {
			plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
					"\nNode %d: Cannot find qpair %p sn %d dn %d ss %d ds %d\n",
					myid, q_pair_MEMBERSHIP, myid, myid == 0 ? 1 : 0, my_protocol, SDF_MEMBERSHIP);
		} else {
			plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
	...
	plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
			"\nNode %d Exiting pthread membership Tester\n", myid);
	return (0);
}
