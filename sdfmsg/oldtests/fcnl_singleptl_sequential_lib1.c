...
/*
 * File:   fcnl_singleptl_sequential_lib1.c
 * Author: mac
 *
 ...
 *
 * @brief: 2 -> 2
 * Create two senders and two receivers on each node, let the sender send
 * message to the receiver. You can change the type of sender and receiver.
 * 
 */
...
/* the sender send two types of protocol messages 
 * the number is set below */

static int msgCount = 26, fthCount = 0;
/* 
...
void
fthThreadSinglePtlSeqSender(uint64_t arg) {
    int l = 0;
    vnode_t node;
...
void fthThreadSinglePtlSeqRecver(uint64_t arg) {
    int ret, ct = 0;
...
void *
SinglePtlSequentialPthreadRoutine(void *arg) {
    ...
    XResume(XSpawn(&fthThreadSinglePtlSeqRecver, 40960), SDF_CONSISTENCY);
    XResume(XSpawn(&fthThreadSinglePtlSeqRecver, 40960), SDF_RESPONSES);

    XResume(XSpawn(&fthThreadSinglePtlSeqSender, 40960), SDF_CONSISTENCY);
    XResume(XSpawn(&fthThreadSinglePtlSeqSender, 40960), SDF_RESPONSES);
