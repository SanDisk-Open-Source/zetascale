...
main(int argc, char *argv[])
{
    ...
    success =
        plat_opts_parse_mpilogme(&config, argc, argv) ? SDF_FALSE : SDF_TRUE;

    printf("input arg %d msgnum %d success %d\n", config.inputarg,
           config.msgtstnum, success);
    fflush(stdout);
    ...
    pthread_create(&fthPthread, &attr, &PerformancePthreadRoutine,
                   (void *)&size);
    ...
    if (myid == 0) {
        sched_yield();
        printf
            ("Node %d: Exiting message test after yielding... Calling MPI_Finalize\n",
             myid);
        fflush(stdout);
        sched_yield();
        MPI_Finalize();
