/*
 * File:   fcnl_queue_order_fth_test2.c
 * Author: Norman Xu
 *
 ...
 *
 * $Id: fcnl_queue_order_fth_test1.c 308 2008-06-23 22:34:58Z normanxu $
 */
...
int main(int argc, char *argv[]) {
    ...
    pthread_create(&fthPthread, &attr, &OrderTestFthSinglePthreadRoutine, NULL);

    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
            "\nNode %d: Created pthread for FTH threads - queue order test\n", myid);

    pthread_join(fthPthread, NULL);

    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
            "\nNode %d: SDF Messaging Test Complete for queue order fth test2\n", myid);
