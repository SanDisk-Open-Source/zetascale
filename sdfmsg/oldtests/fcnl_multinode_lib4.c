...
/*
 * File:   fcnl_multinode_lib4.c
 * Author: mac
 *
 * Created on Aug 25, 2008, 12:39 AM
 *
 ...
 *
 * @brief: 2 -> 2 (singleside, singleprotocol)
 * node 0, node 1 create only one type of protocol sender, other two nodes create only the same type 
 * of receiver. node 0, node 1 send message to other two nodes, you can control whether need response
 * from receiver. 
 */
...
void
fthThreadMultiNodeTstotrSender(uint64_t arg) {
    int l = 0;
    vnode_t node;
    ...
    q_pair[node] = sdf_create_queue_pair(myid, node, ptl, ptl, SDF_WAIT_FTH);// 0 ==> 2, 1 ==> 3
...
void fthThreadMultiNodeTstotrRecver(uint64_t arg) {
    int ret, ct = 0;
    uint64_t aresp = 0;
    ...
    node = arg;
    q_pair[node] = sdf_create_queue_pair(myid, node, ptl, ptl, SDF_WAIT_FTH);// 2 ==> 0, 3 ==> 1
...
void *
MultiNodeTstotrPthreadRoutine(void *arg) {
    ...
    if(myid == 2 || myid == 3) {
        fthResume(XSpawn(&fthThreadMultiNodeTstotrRecver, 40960), myid - 2);
    }
    else {
        fthResume(XSpawn(&fthThreadMultiNodeTstotrSender, 40960), myid + 2);
    }
    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
