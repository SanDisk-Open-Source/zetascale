...
/*
 * File:   fcnl_multinode_doubleside_lib2.c
 * Author: mac
 *
 * Created on Aug 27, 2008, 3:30 PM
 *
 ...
 *
 * @brief: 2 -> 2 (doubleside, singleprotocol) => doubleside means each nodes both send and receive message
 * every node creates only one type of protocol sender and receiver,  node #n sends message to #n-2 or #n+2
 * nodes and receives message from that node, you can control whether need response from receiver. <pnode just 
 * hard code now>. 
 */
...
void *
MultiNodeTntotnPthreadRoutine(void *arg) {
    ...
    if(myid == 2 || myid == 3) {
        XResume(XSpawn(&fthThreadMultiNodeTntotnSender, 40960), myid - 2);// 2 => 0, 3 => 1 
        XResume(XSpawn(&fthThreadMultiNodeTntotnRecver, 40960), myid - 2);// 2 <= 0, 3 <= 1
    }
    else {
        XResume(XSpawn(&fthThreadMultiNodeTntotnSender, 40960), myid + 2);// 0 => 2, 1 => 3 
        XResume(XSpawn(&fthThreadMultiNodeTntotnRecver, 40960), myid + 2);// 0 <= 2, 1 <= 3
    }
    info->pthread_info = 1;
    info->fth_info = 4;
