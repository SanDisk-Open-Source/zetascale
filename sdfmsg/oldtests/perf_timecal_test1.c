 ...
 * Author: Mac
 *
 * Created on Aug 21, 2008, 9:27 AM
 *
...
#define MAX_CORES 1
#define MAX_FTH_THREAD 16
#define MAX_MSG_SIZE 8192
pthread_t fthPthread;
static int ncores;
static int nthreads;
static int niterator;
static int nsize;
int main(int argc, char *argv[]) {
	...
	if (argc > 4) {
		nsize = atoi(argv[4]);
		if (nsize > MAX_MSG_SIZE)
			nsize = MAX_MSG_SIZE;
	}

	arg.ncores = ncores;
	arg.niterators = niterator;
	arg.nmsgsize = nsize;
	...
	pthread_create(&fthPthread, &attr, &PerfTestTimeConsumePthreadRoutine, &arg);
