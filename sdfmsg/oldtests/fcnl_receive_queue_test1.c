...
/*
 * File:   fcnl_receive_queue_test1.c
 * Author: Norman Xu
 *
 ...
 *
 * $Id: fcnl_receive_queue_test1.c 308 2008-06-23 22:34:58Z normanxu $
 */
...
int main(int argc, char *argv[]) {
    ...
    pthread_create(&fthPthread, &attr, &ReceiveQueuePthreadRoutine, &myid);
    info->pthread_info = 1;
    info->fth_info = 2;

    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
            "\nNode %d: Created pthread for receive queue test\n", myid);

    pthread_join(fthPthread, NULL);

    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
            "\nNode %d: SDF Messaging receive queue Test Complete\n", myid);
