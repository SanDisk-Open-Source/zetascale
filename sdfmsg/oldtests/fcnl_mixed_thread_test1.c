...
/*
 * File:   fcnl_mixed_thread_test1.c
 * Author: Norman Xu
 *
 * Created on July 21st, 2008, 7:45AM
 *
 * (c) Copyright 2008, Schooner Information Technology, Inc.
 * http://www.schoonerinfotech.com/
 *
 * $Id: fcnl_mixed_thread_test1.c 308 2008-07-21 22:34:58Z normanxu $
 */

/*!
 * @brief we create this test case to test sending and receiving with
 * both pthread and fth sender and receiver
 */
#ifdef MPI_BUILD
#include <mpi.h>
#endif
...
int main(int argc, char *argv[]) {
    ...
    pthread_create(&fthPthread[0], &attr, &MixedthreadTestfthRoutine, &myid);
    pthread_create(&fthPthread[1], &attr, &MixedthreadTestpthreadRoutine, &myid);
    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
            "\nNode %d: Created pthreads for mixed thread test1\n", myid);
    info->pthread_info = 2;
    info->fth_info = 6;
