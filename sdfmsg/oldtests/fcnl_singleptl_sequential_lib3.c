/*
 * File:   fcnl_singleptl_sequential_lib3.c
 * Author: mac
 *
 * Created on Aug 20, 2008, 5:01 PM
 *
 ...
 * message to the receiver. the sender will get ack and resp message(control
 * by the user), then receiver will give response to sender and ack when 
 * sending success, You can change the type of sender and receiver.
 * 
 */
...
static int msgCount = 10, fthCount = 0;
/* 
...
void *
SinglePtlSequentialPressPthreadRoutine(void *arg) {
    ...
    XResume(XSpawn(&fthThreadSinglePtlPressSeqRecver, 40960), SDF_CONSISTENCY);

    XResume(XSpawn(&fthThreadSinglePtlPressSeqSender, 40960), SDF_CONSISTENCY);
