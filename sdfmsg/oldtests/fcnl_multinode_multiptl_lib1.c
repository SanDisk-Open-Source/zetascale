...
/*
 * File:   fcnl_multinode_multiptl_lib1.c
 * Author: mac
 *
 * Created on Aug 26, 2008, 9:49 AM
 *
 ...
 *
 * @brief: 1 -> x (singleside, multiprotocol)
 * node 0 create only multi type of protocol sender, other nodes create only the different type 
 * of receiver. node 0 sends message to other nodes, you can control whether need response
 * from receiver. Now i just select three or two types protocol. 
 */
...
void
fthThreadMultiNodeMultiPtlSstomrSender(uint64_t arg) {
    int l = 0;
    vnode_t node;
...
void fthThreadMultiNodeMultiPtlSstomrRecver(uint64_t arg) {
    int ret, ct = 0;
    ...
    q_pair[node] = local_create_myqpairs(ptl, myid, node);
...
void *
MultiNodeMultiPtlSstomrPthreadRoutine(void *arg) {
    ...
    if(myid) {
        XResume(XSpawn(&fthThreadMultiNodeMultiPtlSstomrRecver, 40960), myid + 1); // the last parameter type is protocol 
    }
    else {
        for(int index = 1; index < *numprocs; index ++)
            XResume(XSpawn(&fthThreadMultiNodeMultiPtlSstomrSender, 40960), index);// the last parameter type is pnode
    }
    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
