...
/*
 * File:   fcnl_singleptl_chksize_lib1.c
 * Author: mac
 *
 * Created on Aug 1, 2008, 4:48 PM
 *
 ...
 * Let each node create a sendes and a receiver, sender sends message to the peer node's receiver,
 * before the receiver do not fetch message from the q-out
 */
...
static int msgCount = 50, fthCount = 0;//let the program crashed

void
fthThreadSglPtlSizeChkSender(uint64_t arg) {
    int l = 0;
    vnode_t node;
...
#if 1 
        if(myid == 0)
            printf("node %d, sender #%li sends %d times, sdf_msg_send return %d, message contents %d-%li\n", 
                   myid, ptl, l + 1, ret, l, ptl);
        else
            printf("node %d, sender #%li sends %d times, sdf_msg_send return %d, message contents %d-%li\n", 
                   myid, ptl, l + 1, ret, l + msgCount, ptl);
#endif 
...
void fthThreadSglPtlSizeChkRecver(uint64_t arg) {
    int ct = 0;
    uint64_t ptl;
    vnode_t node;
...
void *
SinglePtlQsizeChkPthreadRoutine(void *arg) {
    ...
    XResume(XSpawn(&fthThreadSglPtlSizeChkRecver, 40960), SDF_CONSISTENCY);
    XResume(XSpawn(&fthThreadSglPtlSizeChkSender, 40960), SDF_CONSISTENCY);
    info->pthread_info = 1;
    info->fth_info = 2;
    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
