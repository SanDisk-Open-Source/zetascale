 ...
 * node #0 create only one type of protocol sender and receiver, other nodes create only the same type 
 * of sender and receiver. node 0 sends message to other nodes and receives message from other nodes;
 * other nodes receives message from other nodes and sends message to other nodes,  you can control 
...
void
fthThreadMultiNodeMstomrSender(uint64_t arg) {
    int l = 0;
    vnode_t node;
    ...
    if (myid) {
        ...
        while (fthCount != 2 * (numprocs - 1)) fthYield(10);
    }
...
void fthThreadMultiNodeMstomrRecver(uint64_t arg) {
    int ret, ct = 0;
    uint64_t aresp = 0;
...
//    sdf_msg_startmsg(myid, SDF_MSG_RTF_DISABLE_MNGMT, NULL);
    ...
    if(recv_msg) {
        ...
        fprintf(stderr,"!!node %d, receiver type#%li recvs protocol#%d meessage from sender failed\n", myid, ptl, recv_msg->msg_dest_service);
    }    
#endif
        ...
        ret = sdf_msg_send((struct sdf_msg *)send_msg, TSZE, node, ptl, myid, ptl, RESP_ONE,
                        &fthmbx, sdf_msg_get_response(recv_msg, ptrkey));
...
void *
MultiNodeMstomrPthreadRoutine(void *arg) {
    ...
    if(myid) {
        XResume(XSpawn(&fthThreadMultiNodeMstomrSender, 40960), 0);// 1 => 0, 2 => 0, 3 => 0 
        XResume(XSpawn(&fthThreadMultiNodeMstomrRecver, 40960), 0);// 1 <= 0, 2 <= 0, 3 <= 0
    }
    else {
        ...
        for(int index = 1; index < *numprocs; index ++) {
            XResume(XSpawn(&fthThreadMultiNodeMstomrSender, 40960), index);// 0 => 1, 0 => 2, 0 => 3 
            XResume(XSpawn(&fthThreadMultiNodeMstomrRecver, 40960), index);// 0 <= 1, 0 <= 2, 0 <= 3
        }
    }
    info->pthread_info = 1;
    info->fth_info = (*numprocs)*2+2;
