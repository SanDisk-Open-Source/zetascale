/*
 * File:   fcnl_queue_order_pth_test1.c
 * Author: Norman Xu
 *
 ...
 *
 * $Id: fcnl_queue_order_pth_test1.c 308 2008-06-23 22:34:58Z normanxu $
 */
...
/* There will be a better way from Drew but for now we do it like the sdf/agent */
...
/* borrowed from the sdf/agent shmem init approach */
...
int main(int argc, char *argv[]) {
    ...
    lock_processor(0, 2);
    info->lock_cpu = 2;
    /* Startup SDF Messaging Engine test is run in dual node mode only - pnodeid is passed and determined
    ...
    /* for this pthreads messaging test Node 0 is designated as the sender, every other Node will just listen */

    if (myid == 0) {
        pthread_create(&fthPthread, &attr, &OrderingSenderPthreadRoutine, &myid);
    }
    else {
        pthread_create(&fthPthread, &attr, &OrderingReceiverPthreadRoutine, &myid);
    }

    plat_log_msg(PLAT_LOG_ID_INITIAL, LOG_CAT, PLAT_LOG_LEVEL_TRACE,
            "\nNode %d: Created pthreads for messaging test\n", myid);
    info->pthread_info = 1;
    info->fth_info = 2;
