...
/*
 * File:   fcnl_multinode_test4.c
 * Author: mac
 *
 * Created on Aug 25, 2008, 12:50 PM
 *
...
main(int argc, char *argv[]) {
    ...
    info->myid = myid;
    //Assure the case run properly
    if(numprocs!=4){
    	if(myid==0){
    		printf("*************Error***************\n");
    		printf("You must assign 4 nodes in this case. Use the following  command:\n\tmpirun -np 4 fcnl_multinode_test4\n");
    		MPI_Finalize();
    ...
    pthread_create(&fthPthread, &attr, &MultiNodeTstotrPthreadRoutine, &numprocs);
