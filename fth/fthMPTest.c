...
void threadRoutine3(uint64_t arg) {
    ...
    while (1) {
        ...
        if (mail == 0) {
            ...
            if (mail != 999) {
                exitRV = 1;
                pf("Mbox try fail: expected 999 but got %"PRIu64" after %i tries\n", mail, numTries);
            } else {
                pf("Mbox head wait OK: expected 999 and got %"PRIu64" after %i tries\n", mail, numTries);
            }
            break;
...
void threadRoutine2(uint64_t arg) {
    pf("Thread 2 start - %"PRIu64" number %i\n", arg, __sync_fetch_and_add(&threadCount, 1));

    fthWaitEl_t *waitR = fthLock(&lock1, 0, NULL);   // Read lock

    pf("Thread 2 got read lock - %"PRIu64"\n", arg);
    fthUnlock(waitR);
    pf("Thread 2 released read lock - %"PRIu64"\n", arg);
    fthWaitEl_t *waitW = fthLock(&lock2, 1, NULL);   // Write lock
    pf("Thread 2 got write lock - %"PRIu64"\n", arg);

    fthUnlock(waitW);
    pf("Thread 2 released write lock - %"PRIu64"\n", arg);
    
    if (__sync_fetch_and_add(&threadCount, -1) == 1) {
        pf("Thread 2 last man standing starts thread3 - %"PRIu64"\n", arg);
        fthResume(fthSpawn(&threadRoutine3, 4096), 0);        
        sleep(1);

        pf("Thread posts mailbox singleton - %"PRIu64" (current try count %i)\n", arg, numTries);
...
void *pthreadRoutine(void *arg) {
    ...
    pf("Scheduler %"PRIu64" starting\n", (uint64_t) arg);

    fthSchedulerPthread(0);
    
    pf("Scheduler %"PRIu64" halted\n", (uint64_t) arg);
