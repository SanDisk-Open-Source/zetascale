...
void threadRoutine9(uint64_t arg) {
    pf("Thread 9 start - %"PRIu64"\n", arg);
    fthYield(-1);
    pf("Thread 9 end - %"PRIu64"\n", arg);
}
    
void threadRoutine8(uint64_t arg) {
    pf("Thread 8 start - %"PRIu64"\n", arg);
    fthYield(0);
    pf("Thread 8 Mid - %"PRIu64"\n", arg);
    fthYield(0);
    pf("Thread 8 end - %"PRIu64"\n", arg);
}
    
void threadRoutine7(uint64_t arg) {
    pf("Thread 4 start - %"PRIu64"\n", arg);
    fthNanoSleep(arg);
    pf("Thread 4 end - %"PRIu64"\n", arg);
}
    
void threadRoutine6(uint64_t arg) {
    printf("In threadRoutine6 %"PRIu64"\n", arg);
    fthMutexLock(&mutex);
    printf("mutex wait complete - %"PRIu64"\n", arg);
    fthMutexUnlock(&mutex);
}

void threadRoutine5(uint64_t arg) {
    printf("In threadRoutine5 %"PRIu64"\n", arg);
}

void threadRoutine4(uint64_t arg) {
    printf("In threadRoutine4\n");
    fthWaitEl_t *wait = fthLock(&lock, 0, NULL);
    printf("Got read lock - %"PRIu64"\n", arg);
    fthYield(1);
    fthUnlock(wait);
    printf("Got read unlock - %"PRIu64"\n", arg);
    fthYield(1);
    wait = fthLock(&lock, 1, NULL);
    printf("Got write lock - %"PRIu64"\n", arg);
    fthUnlock(wait);
}

void threadRoutine3(uint64_t arg) {
    printf("In threadRoutine3 %"PRIu64"\n", arg);
    uint64_t mail = fthMboxWait(&mbox);
    if (mail != arg) {
        exitRV = 1;
        printf("Mbox head wait fail: expected %"PRIu64" but got %"PRIu64"\n", arg, mail);
    } else {
        printf("Mbox head wait OK: expected %"PRIu64" and got %"PRIu64"\n", arg, mail);
    }
}
...
void threadRoutineStruct(uint64_t arg) {
    printf("In threadRoutineStruct %"PRIu64"\n", arg);
    dummyStruct_t *ds = (dummyStruct_t *) fthStructQWait(&structQ);
    if (ds->id != arg) {
        exitRV = 1;
        printf("Struct wait fail: expected %"PRIu64" but got %i\n", arg, ds->id);
    } else {
        printf("Struct wait OK: expected %"PRIu64" and got %i\n", arg, ds->id);
    }
}
...
void threadRoutine1(uint64_t arg) {
    ...
    if (mail != 123) {
        exitRV = 1;
        printf("mbox wait fail: expected 123 but got %"PRIu64"\n", mail);
    }

    mail = fthMboxWait(&mbox);
    if (mail != 456) {
        exitRV = 1;
        printf("Mbox head wait fail: expected 456 but got %"PRIu64"\n", mail);
    }

    mail = fthMboxWait(&mbox);
    if (mail != 789) {
        exitRV = 1;
        printf("Mox middle wait fail: expected 789 but got %"PRIu64"\n", mail);
    }
    
    mail = fthMboxWait(&mbox);
    if (mail != 111) {
        exitRV = 1;
        printf("Mbox head wait fail: expected 111 but got %"PRIu64"\n", mail);
    }
    ...
    pf("About to sleep for %"PRIu64" second(s)\n", sleep/1000000000);
    fthNanoSleep(sleep);
    pf("Back from sleep\n");
