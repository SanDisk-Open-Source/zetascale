...
#define MAX_NODES_PER_OBJECT        6700
	...
	do {
		...
		*new_node = n_new;
		...
		if (is_leaf(btree, (*n_child)->pnode)) {
			 ...
			 */
			btree_leaf_find_key2(btree, (*n_child)->pnode, new_key, new_keylen, meta, &index);
		}
		split_key = 0;
		split_count++;
	} while (is_node_full(btree, (*n_child)->pnode, new_key, new_keylen, datalen,
			      meta, syndrome, new_key_exists, index));
		...
		btree_raw_mem_node_t* left, int parent_nkey_child, uint32_t* not_written)
{
	...
	*not_written = count;
	...
	do {
		...
		child_id = BAD_CHILD;
#if 0
		find_key(btree, parent->pnode, objs->key, objs->key_len, meta, 0, &child_id,
			 &child_id_before, &child_id_after, &parent_nkey_child);
#endif 
