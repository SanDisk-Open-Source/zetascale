	...
	)
{
    if (agent_engine_pre_init(&agent_state, argc, argv)) {
        return(agent_engine_post_init(&agent_state));
    } else {
        return(SDF_FAILURE); 
    }
}
    ...
    if (ISEMPTY(cname)) {
       ...
       status = SDF_CONTAINER_EXISTS;
        ...
        if ((shardid = build_shard(state, pai, cname, num_objs,
            ...
            if ((meta = build_meta(cname, properties, *cguid, shardid)) != NULL) {
...
{               
    ...
    char *path = NULL;
    mcd_container_t *mcd_container = NULL;
    ...
    mcd_container = mcd_osd_container_from_cguid(cguid);

    if (NULL == mcd_container) {
	fprintf(stderr, "SDFOpenContainer: NULL mcd_container\n");
        status = SDF_INVALID_PARAMETER;
    } else {
	path = mcd_container->cname;
    }
	...
	)
{
	...
	container = internal_clientToServerContainer(mcd_container->sdf_container);
    }
