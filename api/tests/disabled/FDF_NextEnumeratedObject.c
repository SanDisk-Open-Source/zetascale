...
**********   Function: ZSNextEnumeratedObject
***********************************************/
...
ZS_status_t SetContainerProps(ZS_cguid_t cguid,ZS_boolean_t evicting,ZS_boolean_t persistent,ZS_boolean_t fifo)
{
    ZS_status_t           ret;
...
ZS_status_t ReadObject(ZS_cguid_t cguid,char *key,uint32_t keylen,char **data,uint64_t *datalen)
{
    ZS_status_t           ret;
    ret = ZSReadObject(_zs_thd_state,cguid,key,keylen,data,datalen);
    if(ZS_SUCCESS == ret){
        fprintf(fp,"ZSReadObject cguid=%ld,key=%s success.\n",cguid,key);
        fprintf(fp,"ZSReadObject:key=%s data=%s,dataln=%ld.\n",key,*data,*datalen);
    }
    else fprintf(fp,"ZSReadObject cguid=%ld,key=%s fail:%s.\n",cguid,key,ZSStrError(ret));
    return ret;
}
...
int ZSNextEnumeratedObject_MoreObject1(int count)
{
    ZS_status_t           ret = ZS_SUCCESS;
    int                    flag;
    struct ZS_iterator    *iterator;
...
int ZSNextEnumeratedObject_MoreObject2(int count)
{
    ...
    OpenContainer("x5",ZS_CTNR_CREATE,&cguid);
