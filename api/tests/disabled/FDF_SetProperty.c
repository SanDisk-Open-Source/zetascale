...
FDF_status_t SetContainerProps(FDF_boolean_t evicting,FDF_boolean_t persistent,FDF_boolean_t fifo)
{
    FDF_status_t           ret;
...
int FDFSetProperty_basic_check()
{
...
int FDFSetProperty_Open_CreateObj_close(int count)
{
...
int FDFSetProperty_MoreObject1(int count)
{
    FDF_status_t           ret = FDF_SUCCESS;
    int                    flag;
    struct FDF_iterator    *iterator;
...
int FDFSetProperty_MoreObject2(int count)
{
...
int main(int argc, char *argv[])
{
    ...
    if((fp = fopen("FDF_SetProperty.log", "w+")) == 0){
    ...
    for(int i = 0 ;  i < 3;i++){
        ...
        FDFSetProperty(property,value[i]);
        testCount = 0;
        SetPropMode(eviction[i],persistent[i],fifo[i],writethru[i],async_writes[j],durability[i]);
        OpenContainer("x",FDF_CTNR_CREATE,&cguid);

        result[j][i][0] = FDFSetProperty_basic_check();
        result[j][i][1] = FDFSetProperty_Open_CreateObj_close(2);
//        result[j][i][2] = FDFSetProperty_MoreObject1(6);
//        result[j][i][3] = FDFSetProperty_MoreObject2(5);
    ...
    for(int j = 0;j < 3;j++){
        ...
        for(int i = 0; i < 4; i++){
            if(result[k][j][i] == 1){
                num++;
                fprintf(stderr, "FDFSetProperty %drd success.\n",i+1);
            }
            else if(result[k][j][i] == -2)
                fprintf(stderr, "FDFSetProperty test %drd fail to test.\n",i+1);
            else if(result[k][j][i] == 0)
                fprintf(stderr, "FDFSetProperty test %drd failed.\n",i+1);
            else if(result[k][j][i] == -1)
                fprintf(stderr, "FDFSetProperty test %drd return failed.\n",i+1);
            else fprintf(stderr, "FDFSetProperty test %drd hit wrong.\n",i+1);
        }
    }
    }
    if(resultCount == num){
        fprintf(stderr, "************ test pass!******************\n");
	fprintf(stderr, "#The related test script is FDF_SetProperty.c\n");
	fprintf(stderr, "#If you want, you can check test details in FDF_SetProperty.log\n");
        return 0;
    }
    else 
