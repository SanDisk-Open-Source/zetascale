...
FDF_status_t OpenContainer(char *cname, uint32_t flag, uint32_t dura, uint64_t *cguid)
{
...
FDF_status_t WriteObject(FDF_cguid_t cid, char *key,char *data, uint32_t flags)
{
	FDF_status_t ret;
...
FDF_status_t ReadObject(FDF_cguid_t cid, char *key, char **data, uint64_t *datalen)
{
	FDF_status_t ret;
...
FDF_status_t DeleteObject(FDF_cguid_t cid,char *key)
{
	FDF_status_t ret;
	ret = FDFDeleteObject(fdf_thrd_state,cid,key, strlen(key)+1);
	fprintf(fp,"FDFDeleteObject : %s\n",FDFStrError(ret));
	return ret;
}

FDF_status_t FlushObject(FDF_cguid_t cid,char *key)
{
	FDF_status_t ret;
...
FDF_status_t container_ops(char *cname, uint64_t flag, uint64_t dura)
{
	FDF_status_t ret = FDF_SUCCESS;
	...
	if (FDF_SUCCESS != ret) {
		fprintf(fp, "Delete container failed with: %s\n", FDFStrError(ret));
		goto exit_container_ops;
	}
...
mgr1()
{
	...
	do {
		...
		/*
		 * If we could never generate a loop breaking random number
		 */
		if (MAX_ITERATION == i) {
			break;
		}
