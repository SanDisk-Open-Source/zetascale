...
void launch_blk_write_threads(uint32_t nrows, uint32_t ncols, uint32_t nthreads)
{
...
void do_bulk_test(uint32_t nrows, uint32_t ncols, uint32_t nthreads)
{
	struct FDF_thread_state *thd_state;
	...
	launch_blk_write_threads(nrows, ncols, nthreads);	
...
main(int argc, char *argv[])
{
	...
	printf("Usage: <prog> <num_rows> <num_cols> <num_threads>\n");
	...
	fprintf(stderr, "Starting test with %u rows %u cols in %u threads\n",
	        nrows, ncols, nthreads);

	FDFInit(&fdf_state);
	do_bulk_test(nrows, ncols, nthreads);
