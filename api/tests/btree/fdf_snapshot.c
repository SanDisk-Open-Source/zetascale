...
static void
do_mput(struct FDF_thread_state *thd_state, uint32_t start, uint32_t cnt, int iter, uint32_t overflow_freq)
{
	int i;
	FDF_status_t status;
	FDF_obj_t *objs = NULL; 
	...
	objs = (FDF_obj_t *) malloc(sizeof(FDF_obj_t) * cnt);
	if (objs == NULL) {
	...
	memset(objs, 0, sizeof(FDF_obj_t) * cnt);
	...
	uint32_t flags = 0;
	status = FDFMPut(thd_state, cguid, cnt,
...
static void
do_write(struct FDF_thread_state *thd_state, uint32_t start, uint32_t cnt, int iter, bool overflow_freq)
{
	char *key;
	uint32_t keylen;
	FDF_status_t status;
	...
	for (i = start; i < start + cnt; i++) {
		...
		status = FDFWriteObject(thd_state, cguid, key, keylen,
		                        values[i].value, values[i].vallen, 0);
		if (status != FDF_SUCCESS) {
			printf("FDFWrite failed for key %d\n", i);
			fflush(stdout);
		}
...
static void
do_read_verify(struct FDF_thread_state *thd_state, uint32_t start, uint32_t cnt,
               FDF_status_t exp_status)
{
	char *key;
	uint32_t keylen;
	char *act_value;
	...
	for (i = start; i < start + cnt; i++) {
		...
		if (status != exp_status) {
			printf("FDFRead returned status=%s, expected status=%s for "
			        "key %d\n", FDFStrError(status), FDFStrError(exp_status), i);
			fflush(stdout);
			errors++;
			free(key);
			continue;
...
static void
do_delete(struct FDF_thread_state *thd_state, uint32_t start, uint32_t cnt,
          FDF_status_t exp_status)
{
	char *key;
	uint32_t keylen;
	...
	for (i = start; i < start + cnt; i++) {
		...
		status = FDFDeleteObject(thd_state, cguid, key, keylen);
		if (status != exp_status) {
			printf("FDFDelete returned status=%s, expected status=%s for "
			        "key %d\n", FDFStrError(status), FDFStrError(exp_status), i);
			fflush(stdout);
			free(key);
			errors++;
	...
	if (errors == 0) {
		printf("SUCCESS: FDFDeleted keys successfully\n");
	} else {
		printf("ERROR: FDFDelete Failed\n");
	}
	fflush(stdout);
}
...
static void
create_snapshot(struct FDF_thread_state *thd_state, uint32_t snap_ind)
{
	FDF_status_t status;
                        ...
                        uint32_t snap_ind, int chunk_size)
{
	...
	ret = FDFGetRange(thd_state,
	                  cguid,
	                  FDF_RANGE_PRIMARY_INDEX,
	                  &cursor, 
	                  rmeta);
	if (ret != FDF_SUCCESS) {
		fprintf(stdout, "FDFStartRangeQuery failed with status=%d\n", ret);
		fflush(stdout);
		return;
	}
	free(rmeta);
	...
	do {
		...
		ret = FDFGetNextRange(thd_state,
		                      cursor,
		                      chunk_size,
		                      &n_out,
		                      rvalues);

		if ((n_out != chunk_size) && (ret != FDF_QUERY_DONE)) {
			fprintf(stdout, "Error: Snapshot read chunk size "
			         "expected %d, actual %d\n", chunk_size, n_out);
			fflush(stdout);
			errors++;
		} else if ((ret != FDF_SUCCESS) && (ret != FDF_QUERY_DONE)) {
...
freeup:
	...
	ret = FDFGetRangeFinish(thd_state, cursor);
	if (ret != FDF_SUCCESS) {
		fprintf(stdout, "ERROR: FDFGetRangeFinish failed ret=%d\n", ret);
		fflush(stdout);
		errors++;
	}
...
//	uint32_t iter_print_cnt = 0;
	...
	for (i = 0; i < num_snaps; i++) {
		...
		status = FDFWriteObject(thd_state, cguid, key, keylen, new_data, act_vallen, 0);
		if (status != FDF_SUCCESS) {
			printf("WriteObject failed (%d) error: %s\n", i, FDFStrError(status));
			break;
		}
