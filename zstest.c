...
int main(int argc, char **argv)
{
    ...
    if (Verbose) {
	printf("===============================\n");
	printf("Read Response Time Statistics:\n");
	printf("===============================\n");
	process_stats(&rtstats_all_read);
	printf("===============================\n");
	printf("Write Response Time Statistics:\n");
	printf("===============================\n");
...
static void *test_routine(void *arg)
{
    ...
    dump_thresh  = iters_per_thread/100;
    if (dump_thresh == 0) {
	dump_thresh = 1;
    }
...
static void dump_progress(uint64_t i, uint64_t iters_per_thread, uint32_t n_keys_written, uint64_t objs_per_thread)
{
    ...
    (void) fprintf(stderr, " %5.3g%% (%8.2g kIOPs) %8d distinct objects (%7.3g %%)\n", floor(0.5+100.*((double) i)/iters_per_thread), ((double) aio_read_iops - last_read_iops + aio_write_iops - last_write_iops)/dt/1000., n_keys_written, 100.0*((double) n_keys_written)/((double) objs_per_thread));
...
static void process_stats(rt_statistics_t *rtstats)
{
    ...
    printf("------------------------------------------------------------------\n");
    printf("Values are in milliseconds!\n\n");
    z = 0;
    for (i=0; i<64; i++) {
        x = 1ULL << i;
        x *= 2;
	...
	printf("%10.4g (%7.5g):", x/(cpu_mhz*1000), z/n*100.0);
