...
/*
 * File:   $URL: svn://svn/schooner-trunk/trunk/sdf/platform/aio_error_control.h $
 * Author: drew
 *
 ...
 * @param error_control <IN> paio_error_control returned from appropriate
 * paio_api create call like #paio_error_bdb_create.
 * @param fd <IN> file descriptor for error
 * @param error_type <IN> type of error
 * @param start <IN> error region start offset
 * @param len <IN> error region length
 * @return 0 on success, -errno on failure
 */
int paio_error_control_set_error_fd(struct paio_error_control *error_control,
                                    enum paio_ec_type error_type, int fd,
                                    off_t start, off_t len);
