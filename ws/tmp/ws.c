...
int WSWriteAllocate(struct ws_state *ps, void *pzst, char *key, uint32_t key_len, uint64_t size)
{
    ...
    if (overflow) {
	TRACE(ps, "... WSWriteAllocate key=%"PRIu64"; newbuf=%p, p=%lld, size=%lld, new_stripe=%"PRIu64"", keynum, WSNewBuf, WSCurPtr, WSCurSize, WSNewBuf->n_stripe);
    } else {
	TRACE(ps, "... WSWriteAllocate key=%"PRIu64" OVERFLOW; curbuf=%p, p=%lld, size=%lld, stripe=%"PRIu64"", keynum, ps->curbuf, WSCurPtr, WSCurSize, ps->curbuf->n_stripe);
    }
    return(WS_OK);
}
...
static int do_stripe_read(ws_state_t *ps, uint64_t n_stripe, void *buf)
{
    ...
    DOSTAT(ps, NULL, N_IO_STRIPE_READ_BYTES, ps->stripe_bytes);
    ...
    sze = pread(ps->config.fd_devices[0], buf, ps->stripe_bytes, addr + ps->config.device_offset_mb*1024*1024);
    // xxxzzz improve error handling
...
static int do_gc(ws_state_t *ps, ws_stripe_buf_t *sbuf)
{
	...
	if (cfg->gc_cb(ps->config.cb_state, sbuf->buf, &(sbuf->p), ps->stripe_bytes, ps->sector_bytes, addr_from_stripe(ps, sbuf->n_stripe), &cleaned_bytes)) {
	    //  go into read-only mode!
