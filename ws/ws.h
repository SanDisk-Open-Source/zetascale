...
//  Callback for GC
   ...
   *    WS_ERROR
   */
typedef int (*ws_gc_cb_t)(void *state, void *pbuf, uint64_t *p_out, uint32_t stripe_bytes, uint32_t sector_bytes, uint64_t stripe_addr, uint64_t *compacted_bytes, uint64_t device_offset);
...
typedef struct ws_config {
    ...
    ws_gc_cb_t                gc_phase1_cb;
    ws_gc_cb_t                gc_phase2_cb;
