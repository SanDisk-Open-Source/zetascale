	...
	uint64_t src_blk_offset)
{
	...
	uint32_t i;
	char *buf, _buf[2 * Mcd_osd_blk_size];
	uint64_t dst_blk_offset, target_seqno = 0, seqno;
	uint64_t dst_offset, src_offset, idx, syndrome = 0;
	fthWaitEl_t               * wait;
	mcd_logrec_object_t log_rec;

	seqno = atomic_inc_get(shard->sequence);

	buf = (char *)( ( (uint64_t)_buf + Mcd_osd_blk_size - 1 )
                    & Mcd_osd_blk_mask );
	...
	src_offset = mcd_osd_rand_address(shard, src_blk_offset);
	dst_offset = mcd_osd_rand_address(shard, dst_blk_offset);

	for(i = 0; i < class->slab_blksize; i++)
	{
		if((rc = mcd_fth_aio_blk_read(context,
				buf,
				src_offset + i * Mcd_osd_blk_size,
				Mcd_osd_blk_size)))
			goto out;
		...
		if(!i)
		{
			...
			syndrome = hash((unsigned char*)buf + sizeof(mcd_osd_meta_t), meta->key_len, 0 );
		...
		if((rc = mcd_fth_aio_blk_write(context,
					buf,
					dst_offset + i * Mcd_osd_blk_size,
					Mcd_osd_blk_size)))
			goto out;

		stat_inc(shard, BLOCKS_RELOCATED);
    }
	...
	mcd_osd_slab_class_t* class)
{
	...
	{
		map_value = segment->alloc_map[map_offset];

		i = 64;
		while(map_value)
		{
			if((map_value & 0x1) && slab_gc_relocate_slab(context, shard, class, blk_offset))
			{
