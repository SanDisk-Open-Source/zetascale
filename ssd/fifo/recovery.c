 ...
 */
static int bit_bytes(int n);
static int all_done(rec_t *rec);
static int sect_read(sur_t *sur);
static int coal_alloc(rec_t *rec);
static int sf_next(slab_free_t *sf);
static int coal_puthash(rec_t *rec);
static int sf_avail(slab_free_t *sf);
static int ctr_copy_cache(rec_t *rec);
static int almost_full(mo_shard_t *shard);
static int printable(char *ptr, uint_t len);
static int ssx_lock(cntr_t *cntr, uint_t n);
static int bit_count(bitmap_t *map, uint_t n);
static int bit_isset(bitmap_t *map, uint_t n);
static int coal_set(rec_t *rec, mobj_t *mobj);
static int msg_rep_bmap_check(sdf_msg_t *msg);
static int msg_rep_sect_check(sdf_msg_t *msg);
static int unlock_iret(wait_t *wait, int ret);
static int obj_prep(sur_t *sur, uint_t max_obj);
static int do_sect(rec_t *rec, sp_msg_type_t type);
static int sect_unit_l2b(mo_shard_t *shard, uint_t segno);
static int ctr_copy_flash(rec_t *rec, mrep_bmap_t *mrep_bmap);
static int fifo_set_map(mo_shard_t *shard, bitmap_t *seg_map);
static int slab_set_map(mo_shard_t *shard, bitmap_t *seg_map);
static int bit_isset_range(bitmap_t *map, uint_t lo, uint_t hi);
static int slab_alloc(rec_t *rec, slab_t nslab, uint64_t *blkno);
static int ctr_copy(vnode_t rank, struct shard *shard, pai_t *pai);
static int msg_rep_check(sdf_msg_t *msg, mrep_type_t type, int size);
static int obj_valid(mo_shard_t *shard, mo_meta_t *meta, addr_t addr);
static int sf_find_free(slab_free_t *sf, int req_len, slab_t *slab_ptr);
static int read_fifo(sur_t *sur, char *buf, blkno_t blkno, blkno_t nblks);
static int read_disk(sur_t *sur, char *buf, blkno_t blkno, blkno_t nblks);
static int sect_alloc(sect_t *sect, int meta_size, int min_slabs,
                      int boundary, int wait, char *desc);

static void  unused(void);
static void  sf_exit(void);
static void  fth_stop(void);
static void  aioctx_exit(void);
static void  progress(rec_t *rec);
static void  rec_done(rec_t *rec);
static void  rsx_done(rec_t *rec);
static void  fth_one(uint64_t arg);
static void  fth_big(uint64_t arg);
static void  h2n_mrep_h(mrep_t *m);
static void  n2h_mrep_h(mrep_t *m);
static void  send_done(rec_t *rec);
static void  coal_flush(rec_t *rec);
static void  h2n_mobj(mobj_t *mobj);
static void  n2h_mobj(mobj_t *mobj);
static void  cntr_free(cntr_t *cntr);
static void  ssx_unlock(cntr_t *cntr);
static void  nop_fill(sdf_hfnop_t *nop);
static void  post_big(freq_big_t *freq);
static void  post_one(freq_one_t *freq);
static void  fth_init(mo_shard_t *shard);
static void  kl_unlock(klock_t key_lock);
static void  show_stats(int is_recoverer);
static void  aioctx_free(aioctx_t *aioctx);
static void  h2n_mreq_bmap(mreq_bmap_t *m);
static void  h2n_mreq_sect(mreq_sect_t *m);
static void  n2h_mreq_bmap(mreq_bmap_t *m);
static void  n2h_mreq_sect(mreq_sect_t *m);
static void  sf_flush_seg(slab_free_t *sf);
static void  h2n_mreq_h(sp_msg_tiny_t *tiny);
static void  n2h_mreq_h(sp_msg_tiny_t *tiny);
static void  post_freq_one(freq_one_t *freq);
static void  bit_clr(bitmap_t *map, uint_t n);
static void  bit_set(bitmap_t *map, uint_t n);
static void  sf_flush_shard(mo_shard_t *shard);
static void  mobj_show(char *msg, mobj_t *mobj);
static void  show_var(uint64_t val, char *name);
static void  ssx_del(cntr_t *cntr, setx_t setx);
static void  post_mobj(rec_t *rec, mobj_t *mobj);
static void  rsx_send(rec_t *rec, uint64_t head);
static void  rsx_wait(rec_t *rec, uint64_t head);
static void  bless_n(sdf_msg_t *msg, size_t size);
static void  coalesce(rec_t *rec, mlist_t *mlist);
static void  onebyone(rec_t *rec, mlist_t *mlist);
static void  slab_free(rec_t *rec, slab_t nslabs);
static void  h2n_mrep_bmap(mrep_bmap_t *mrep_bmap);
static void  h2n_mrep_sect(mrep_sect_t *mrep_sect);
static void  mrep_sect_show(mrep_sect_t *msg_sect);
static void  n2h_mrep_bmap(mrep_bmap_t *mrep_bmap);
static void  n2h_mrep_sect(mrep_sect_t *mrep_sect);
static void  sur_clean(int rank, mo_shard_t *shard);
static void  liveness(int live, int rank, void *arg);
static void  free_n(void *p, size_t size, char *desc);
static void  coalesce_same(rec_t *rec, mlist_t *mlist);
static void  coal_group_flush(rec_t *rec, setx_t setx);
static void  prep_del(vnode_t rank, struct shard *shard);
static void  obj_layout(mobj_t *mobj, char *ptr, int size);
static void  group_add(obj_group_t *group, freq_one_t *freq);
static void  rsx_put(rec_t *rec, setx_t *setx, int num_setx);
static void  fth_req_big(freq_big_t *freq, aioctx_t  *aioctx);
static void  fth_req_one(freq_one_t *freq, aioctx_t  *aioctx);
static void  set_mreq_h(sp_msg_tiny_t *h, sp_msg_type_t type);
static void  bit_clr_range(bitmap_t *map, uint_t lo, uint_t hi);
static void  bit_set_range(bitmap_t *map, uint_t lo, uint_t hi);
static void  rec_init(rec_t *rec, pai_t *pai, vnode_t rank, struct shard *s,
                      sect_t *sect);
static void  msg_recv(sdf_msg_t *req_msg, pai_t *pai, pas_t *pas,
                      flash_t *flash);
static void  obj_show(char *msg, char *key,
                      int keylen, char *data, int datalen);
static void  intersect(blkno_t *bn_p, blkno_t *nb_p,
                       blkno_t bn1, blkno_t nb1, blkno_t bn2, blkno_t nb2);
static void *malloc_n(size_t size, char *desc);
static void *malloc_nfw(size_t size, char *desc);
static void *realloc_nq(void *p, size_t new_size, size_t old_size);
static char *cache_fill(s_cache_enum_t *cenum);
static char *flash_msg_type(sp_msg_type_t type);
static char *obj_copy(sur_t *sur, char *dst, char *src, klock_t klock);

static setx_t   ssx_need(cntr_t *cntr);
static wait_t  *cntr_lock(sur_t *sur, int force);
static setx_t   ssx_add(cntr_t *cntr, klock_t dlock);
static rklc_t  *get_lock_container(pas_t *pas, mo_shard_t *shard);
static klock_t  kl_lock(sur_t *sur, char *key, uint_t key_len);
static blkno_t  rand_map(mo_shard_t *shard, blkno_t blkno,
                         blkno_t want_blks, blkno_t *cont_p);
static mlist_t *mrep_sect_obj_ready(mrep_sect_t *mrep_sect);
static mlist_t *mlist_pick(mlist_t *mlist, uint_t min_size,
                           uint_t max_size, uint_t slab_l2b);

static sdf_msg_t *mkmsg_fbmap(sur_t *sur);
static sdf_msg_t *mkmsg_cache(sur_t *sur);
static sdf_msg_t *mkmsg_flash(sur_t *sur);
static sdf_msg_t *get_remote_bmap(rec_t *rec);
static sdf_msg_t *mreq_cache(sur_t *sur, sdf_msg_t *req_msg);
static sdf_msg_t *mreq_fbmap(sur_t *sur, sdf_msg_t *req_msg);
static sdf_msg_t *mreq_rdone(sur_t *sur, sdf_msg_t *req_msg);
static sdf_msg_t *mreq_flash(sur_t *sur, sdf_msg_t *req_msg);
static sdf_msg_t *mreq_setix(sur_t *sur, sdf_msg_t *req_msg);
static sdf_msg_t *req_to_rep(sur_t *sur, sdf_msg_t *req_msg);
static sdf_msg_t *mkmsg_null(mrep_type_t type, rerror_t error);
static sdf_msg_t *mkmsg_sect_null(sect_t *sect, rerror_t error);
static sdf_msg_t *get_remote_sect(rec_t *rec, sp_msg_type_t type);
static sdf_msg_t *remote_req_to_rep(vnode_t rank, sdf_msg_t *msg);
static sdf_msg_t *mreq_sect(sur_t *sur, mrep_type_t type, sdf_msg_t *msg);

static aioctx_t    *aioctx_alloc(void);
static freq_one_t  *make_one(rec_t *rec, mobj_t *mobj);
static slab_free_t *sf_get(mo_shard_t *shard, int slab_l2b);
...
fth_req_big(freq_big_t *freq, aioctx_t  *aioctx)
{
    ...
    while (nblks) {
        ...
        s = mcd_fth_aio_blk_write((osd_state_t *) aioctx, p, mapblk*BLK_SIZE, n*BLK_SIZE);
        if (s == FLASH_EOK)
...
obj_show(char *msg, char *key, int keylen, char *data, int datalen)
{
    ...
    /*  This should match sizeof(object_data_t) from memcached
     *  I can't use it directly because SDF is no longer tightly
     *  coupled to memcached.
     */
...
sect_read(sur_t *sur)
{
    ...
    if (shard->use_fifo)
        ...
        return read_disk(sur, buf, blkno, nblks);
}
...
read_fifo(sur_t *sur, char *buf, blkno_t blkno, blkno_t nblks)
{
     ...
     */
    if (!read_disk(sur, buf, blkno, nblks))
        return 0;
     ...
     * is called, we will want to perform the read_disk even though we have
     * done the memcpy.
     */
    for (i = 0; i < MCD_OSD_NUM_WBUFS; i++) {
        struct incore      *c = &incore[i];
        ...
        if (c->offset != wbuf->blk_offset)
            if (!read_disk(sur, &buf[off], c->blkno, c->nblks))
                return 0;
    }
    return 1;
}
 ...
 */
static int
read_disk(sur_t *sur, char *buf, blkno_t blkno, blkno_t nblks)
{
    char *p = buf;
    ...
    while (nblks) {
        int s;
        blkno_t n;
        blkno_t mapblk = rand_map(sur->shard, blkno, nblks, &n);

        sdf_logt(70023, "recovery read %ld:%ld (%ld)", blkno, n, mapblk);
        s = mcd_fth_aio_blk_read((osd_state_t *) sur->aioctx, p, mapblk*BLK_SIZE, n*BLK_SIZE);
        if (s != FLASH_EOK) {
