...
FDF_status_t fdf_start_scavenger_thread(struct FDF_state *fdf_state )
{
    ...
    if(getProperty_Int("FDF_TRX", 0)) {
        plat_log_msg(160236, PLAT_LOG_CAT_PRINT_ARGS, PLAT_LOG_LEVEL_WARN,
                "FDF_EXPIRY_SCAVENGER_ENABLE is incompatible with FDF_TRX. Disabling FDF_EXPIRY_SCAVENGER_ENABLE.");
        return FDF_FAILURE;
    }

    if( fdf_state == NULL ) {
        plat_log_msg(160237, LOG_CAT, LOG_ERR, 
                "invalid fdf state.");
        return FDF_FAILURE; 
    }
    ...
    if((pthread_t)NULL == scav_thd) { //from FDFInit()
        ...
        if( 0 == pthread_create(&scav_thd,NULL, 
            ...
            plat_log_msg(160238, LOG_CAT, LOG_DBG, 
                    "scavenger thread create success.");
            return FDF_SUCCESS;
        }

        plat_log_msg(160239, LOG_CAT, LOG_ERR, 
        ...
        if(0 == pthread_cond_broadcast(&scav_cv)) {
            plat_log_msg( 160240, LOG_CAT, LOG_DBG, 
                    "woke up scavenger.");
            return FDF_SUCCESS;
        } else {
            plat_log_msg( 160241, LOG_CAT, LOG_ERR, 
...
{
    plat_log_msg( 160242, LOG_CAT, LOG_DBG, 
...
static void *scavenger_handler(void *arg)
{
    ...
    while(1){   
        ...
        if( s != FDF_SUCCESS) {
            plat_log_msg(160245, LOG_CAT, LOG_ERR, 
        ...
        pthread_cond_timedwait(&scav_cv, &scav_mutex, &abstime);
        pthread_mutex_unlock(&scav_mutex);
        plat_log_msg(160255, LOG_CAT, LOG_DBG, 
