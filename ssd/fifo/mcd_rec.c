...
update_class( mcd_osd_shard_t * shard, mcd_osd_slab_class_t * class, mcd_osd_segment_t* segment, int del)
{
	...
	char		      * data_buf = NULL, * pclass_data_buf = NULL;
	uint64_t			buf_size = Mcd_osd_segment_size;
	...
	pclass = (mcd_rec_class_t *)pclass_buf;
                   ...
                   mcd_rec_log_state_t * log_state )
{
    ...
    for ( s = 0, blk_count = 0;
        ...
        if(pot_checksum_enabled) {
            ...
            {
                if(sum || !empty(state->segments[s], Mcd_rec_update_segment_size)) {
...
//            hdr->LSN         = ++LSN;
		...
		if (shard->id == cmc_shardid)		// suppress logging for CMC
			...
			rc = mcd_fth_aio_blk_write_low( context, logbuf->buf, offset, MCD_REC_LOGBUF_BLKS * MCD_OSD_META_BLK_SIZE, shard->data_sync);
		if ( FLASH_EOK != rc ) {
