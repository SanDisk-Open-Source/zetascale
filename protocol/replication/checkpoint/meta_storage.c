 ...
 *
 * $Id: meta_storage.c 13386 2010-05-03 09:52:34Z drew $
 */
...
struct rms_shard {
    ...
    /**
     * @brief Meta-data is for split-brain resolution
     *
     * XXX: drew 2010-05-03 This is a hack to keep the RRTF_META_DISTRIBUTED
     * rules which may be compatible with previous versions.
     *
     * Notifications are usually only performed where this node was the
     * meta-data write_node; although in that case the writing node will
     * be the remote node and needs to be the remote node so that the non-lease
     * will supercede the meta-data on the remote end.
     *
     * We need to switch to the seqno + write_node scheme and get away from
     * this mess.
     */
    unsigned shard_meta_split_brain_resolution : 1;
...
struct rms_op {
    ...
    /** @brief Sets shard->shard_meta_split_brain_resolution */
    unsigned split_brain_resolution : 1;
...
rms_shard_put_complete(struct rms_shard *shard, struct rms_op *op,
    ...
    if (status == SDF_SUCCESS) {
        ...
        if (op->split_brain_resolution) {
            shard->shard_meta_split_brain_resolution = 1;
        }
        rms_shard_schedule_lease_timeout(shard);
        rms_shard_notify(shard);
    /*
...
rms_shard_notify_node_by_msg(struct rms_shard *shard, vnode_t node) {
    ...
    } else if (!shard->shard_meta_split_brain_resolution &&
               shard->shard_meta_reput && node != shard->rms->config.my_node) {
...
rms_op_allowed(struct rms_op *op) {
    ...
    /*
     * Correct split-brain problem by setting local node to no lease and
     * over-writing existing lease on other node with no lease so that both
     * sides will back-off and recover.
     *
     ...
     */
        ...
        op->shard_meta->persistent.current_home_node = CR_HOME_NODE_NONE;
        op->shard_meta->persistent.lease_usecs = 0;
        op->split_brain_resolution = 1;
