 ...
 *
 * $Id: copy_replicator_meta.c 8796 2009-08-19 00:21:54Z drew $
 */
...
static int cr_vip_meta_find_shard(struct cr_vip_meta *ids, SDF_shardid_t sguid);
...
cr_shard_meta_dup(const struct cr_shard_meta *rhs) {
    ...
    if (!failed) {
        *lhs = *rhs;
        ...
        /* XXX: drew 2009-09-02 Always use shard_ids structure */
        if (lhs->vip_meta) {
...
cr_vip_meta_alloc() {
    ...
    if (!failed) {
        ret->persistent->header.magic.integer = CR_SHARD_META_MAGIC;
        ret->persistent->header.len = len;
        ret->persistent->header.version = CR_VIP_GROUP_VERSION;
...
/** @brief Return index of sguid in ids, current sguid count */
static int
...
cr_persistent_vip_meta_len(int count) {
    struct cr_persistent_vip_meta *ids = NULL;

    return (sizeof (*ids) + count * sizeof (ids->shards[0]));
}
...
cr_vip_meta_marshal(void **out, size_t *out_len, size_t buffer_offset,
    ...
    if (!buf) {
        ...
        memcpy(buf, in->persistent, in->persistent->header.len);
        ret = SDF_SUCCESS;
    }
...
cr_vip_meta_unmarshal(struct cr_vip_meta **out, const void **inout,
    ...
    if (ret == SDF_SUCCESS) {
        memcpy(&header, inout, sizeof (header));
        if (header.magic.integer != CR_VIP_GROUP_MAGIC) {
            ret = SDF_META_DATA_INVALID;
            reason = "magic";
        } else if (header.version > CR_VIP_GROUP_VERSION) {
...
cr_vip_meta_shard_state_to_string(enum cr_vip_meta_shard_state state) {
    switch (state) {
#define item(caps, lower) case caps: return (#lower);
    CR_VIP_GROUP_SHARD_STATE_ITEMS()
#undef item
    }
    plat_assert(0);
}
