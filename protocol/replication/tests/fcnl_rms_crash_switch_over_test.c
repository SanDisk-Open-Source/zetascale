/*
 * File: fcnl_rms_crash_switch_over_test.c
 * Author: Zhenwei Lu
 *
 * Scenario:
...
user_operations_rms_put_crash_expire_test(uint64_t args) {
    ...
    /* put shard meta to node 2 */
    ++in->persistent.shard_meta_seqno /* for expire and put */;
    ++in->persistent.ltime;
    in->persistent.current_home_node = 2;
    plat_log_msg(LOG_ID, LOG_CAT, LOG_DBG, "put shard meta on node 2");
