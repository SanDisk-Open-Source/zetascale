/*
 * File:   $URL: svn://svn.schoonerinfotech.net/schooner-trunk/trunk/sdf/protocol/replication/tests/fcnl_framework_recover_test.c $
...
user_operations_sync(uint64_t args) {
    ...
    plat_log_msg(LOG_ID, LOG_CAT, LOG_DBG, "read on node 2");
    op_ret = rtfw_read_sync(test_framework, shard_id, 2, key, key_len,
                            &data_out, &data_len_out, &free_cb);
    plat_log_msg(LOG_ID, LOG_CAT, LOG_DBG, "read on node 2 complete");
    plat_assert(op_ret == SDF_SUCCESS);
    plat_assert(strcmp(data_out, data) == 0);
    plat_closure_apply(replication_test_framework_read_data_free_cb, &free_cb,
                       data_out, data_len_out);
    ...
    key_len = strlen(key) + 1;
    plat_log_msg(LOG_ID, LOG_CAT, LOG_TRACE,
                 "write on node 2 key:%s, key_len:%u, data:%s, data_len:%u",
                 key, (int)(strlen(key)), data, (int)(strlen(data)));
    op_ret = rtfw_write_sync(test_framework, shard_id /* shard */, 2 /* node */,
                             meta /* test_meta */, key, key_len, data,
                             strlen(data) + 1);
    plat_log_msg(LOG_ID, LOG_CAT, LOG_DBG, "write on node 2 complete");
    plat_assert(op_ret == SDF_SUCCESS);
...
int main(int argc, char **argv) {
    ...
    opts_config.common_config.test_config.nnode = 3;
