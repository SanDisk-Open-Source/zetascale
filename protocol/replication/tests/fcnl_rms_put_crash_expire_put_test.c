...
/*
 * File: fcnl_rms_put_crash_expire_put_test.c
 * Author: Zhenwei Lu
 *
 * put shard meta => crash node0, 1 => start node0, 1 =>
 * lease expire => get shard meta => put shard meta
 *
...
void
user_operations_rms_put_crash_expire_test(uint64_t args) {
    ...
    /* get meta on node 0 to verify shard meta persistence */
    plat_log_msg(LOG_ID, LOG_CAT, LOG_DBG, "first get on node 0");
    status = rtfw_get_shard_meta_sync(test_framework, 0,
                                      in->persistent.sguid,
                                      r_shard_meta, &out, &expires);
    ...
    /* put shard meta to node 1 */
    ++in->persistent.shard_meta_seqno;
    plat_log_msg(LOG_ID, LOG_CAT, LOG_DBG, "put shard meta on node 1");
...
int main(int argc, char **argv) {
    ...
    XResume(fthSpawn(&user_operations_rms_put_crash_expire_test, 40960), (uint64_t)test_framework);
