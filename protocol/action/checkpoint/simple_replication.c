 ...
 *
 * $Id: simple_replication.c 11990 2010-03-06 04:44:50Z drew $
 */
...
ssdaio_ctxt_t * (*simple_replication_init_ctxt) (void) = NULL;
...
SDF_status_t rpc_release_vip_group(SDF_context_t ctxt, SDF_cguid_t cguid,
                                   ...
                                   vnode_t dest_node,
                                   SDF_shardid_t shard);
static int64_t rpc_get_clock_skew(SDF_context_t ctxt, vnode_t src_node, vnode_t dest_node,
                                  SDF_shardid_t shard, SDF_cguid_t cguid);
...
void print_container_info( qrep_ctnr_state_t *pcs ) {
    ...
    plat_log_msg(21161, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
...
void print_node_info( qrep_node_state_t * pns ) {
    ...
    plat_log_msg(21162, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
                "GroupId:%d\nhName:%s\nNumCnts:%d\nNumIfs:%d\nIfs:%s",
...
void print_group_info( qrep_cluster_grp_t * pgs ) {
    ...
    plat_log_msg(21163, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
...
/*
    ...
    qrep_state_t      *ps;
    ps = &(pas->qrep_state);
    plat_log_msg(20823, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
                     "Number Of Nodes:%d\nStatus Port:%d\nClusterId:%d\nClusterName:%s\nNumGroups:%d\n",
...
void print_sdf_vip_group( sdf_vip_group_t *vgrp ) {
   ...
   plat_log_msg(21164, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL, "VIPGROUP:%s",print_str);
}
...
void print_sdf_vip_group_group( sdf_vip_group_group_t *vggrp ) {
   ...
   plat_log_msg(21165, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL, "VIPGROUPGROUP:%s",print_str);
}
...
void print_sdf_vip_config( sdf_vip_config_t *vipcfg ) {
    ...
    if( vipcfg == NULL ) {
        plat_log_msg(21166, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL, "VIP CONFIG NULL:%s",print_str);
        return;
    }
...
void simple_replication_init(SDF_action_state_t *pas)
{
	...
	plat_log_msg(21169, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
    ...
    for( i = 0; i < lnode->nctnrs_node; i++ ) {
        if( lnode->cntrs[i].cguid != cguid ) {
            if( j >= (lnode->nctnrs_node -1) ) {
                ...
                plat_log_msg(21221, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
       ...
       for( j = 0; j < grp->num_nodes; j++ ) {
          ...
          for( k = 0; k < node->nctnrs_node; k++ ) {
              if( node->cntrs[k].id == cid ) {
                   ...
                   plat_log_msg(21230, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
...
static void simple_replicator_node_dead(SDF_action_state_t *pas, vnode_t pnode)
{
    ...
    plat_log_msg(21245, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
    ...
    for (i = 0; i < nctnrs_node; i++) {
        ...
        if( mynode != master ) {
            ...
            if( pcs->flags & qr_persist  ) {
                ...
                /* ignore replicating if recovery type is only Non Persitent */
...
static void sync_container_thread(uint64_t arg) {
    ...
    paio_ctxt = simple_replication_init_ctxt();
    ...
    while (1) {
        ...
        if (rc == SDF_LOCK_RESERVED) {
            ...
            if (rc != FLASH_EOK && rc != FLASH_EEXIST) {
                plat_log_msg(21279, PLAT_LOG_CAT_SDF_SIMPLE_REPLICATION, PLAT_LOG_LEVEL_FATAL,
