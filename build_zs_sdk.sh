...
then
    ...
    cp -r $SDK_DIR ./zs_sdk
    #tmporaly change fdfdll.a to zsdll.a
    cp ./zs_sdk/lib/libfdfdll.a ./zs_sdk/lib/libzsdll.a
    cp ./zs_sdk/lib/libfdf.so ./zs_sdk/lib/libzs.so

	export FDF_LIB=$PWD/zs_sdk/lib/libzs.so
    mvn clean && mvn install -Dmaven.test.skip=true
    cp target/fdf-*.jar zs_sdk/lib/
...
if [ -n "$run_tests" ]; then
	...
	export FDF_LIB=$WD/output/lib/libzs.so
